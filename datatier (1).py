...
def select_one_row(dbConn, sql, parameters = None):
  ...
  try:# error catch
    if(parameters != None):# check if any ? must be filled
      dbCursor.execute(sql, parameters)#execute
      row = dbCursor.fetchone()#get row
      if(row != None):#check for any data
        return row;
      else:
        return tuple();
    else:
...
def select_n_rows(dbConn, sql, parameters = None):
  ...
  try: # error catch
    if(parameters != None):# check for ?s
...
def perform_action(dbConn, sql, parameters = None):
  ...
  try:# error catch
    if(parameters != None):# check for any ?s
      dbCursor.execute(sql, parameters)#execute
      dbConn.commit()#update table
      return dbCursor.rowcount # return new table size
    else:
      dbCursor.execute(sql)
      dbConn.commit()
